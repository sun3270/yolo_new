...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        loss_name = getattr(self, "coffee_loss_name", "native")
        loss_config = getattr(self, "coffee_loss_config", None)
        if loss_name != "native":
            from loss.coffee_loss import CoffeeE2ELoss, CoffeeLoss

            coffee_file = Path(__import__("loss.coffee_loss", fromlist=["__file__"]).__file__).resolve()
            if "coffee26n_experiment" not in str(coffee_file):
                raise RuntimeError(f"Coffee loss import escaped experiment package: {coffee_file}")
            if getattr(self, "end2end", False):
                return CoffeeE2ELoss(self, profile=getattr(self, "coffee_profile", {}), config=loss_config or {})
            return CoffeeLoss(self, profile=getattr(self, "coffee_profile", {}), config=loss_config or {})
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in frozenset({ResBridge, DetailBlock, MixDown, MidBlock, ContextBlock}) and n != 1:
            raise ValueError(f"{m.__name__} requires repeat n=1, got n={n}.")
        ...
        if m in base_modules:
            ...
            if m is C2fCIB:
                legacy = False
        elif m in frozenset({ELTEB, BoundedELTEB, ELTEBLite}):
            ...
            n = 1
            legacy = False
        elif m in frozenset({EdgeLGMSFBridge, ResidualEdgeLGMSFBridge}):
            if not isinstance(f, list) or len(f) != 2:
            ...
            c2 = c5
            args = [c3, c5, *args]
        elif m is DistanceAwareP2P5Bridge:
            if not isinstance(f, list) or len(f) != 2:
                raise ValueError("DistanceAwareP2P5Bridge expects from=[P2_INDEX, P5_INDEX].")
            c2_near, c5_far = ch[f[0]], ch[f[1]]
            c2 = args[0] if len(args) > 0 else c5_far
            args = [c2_near, c5_far, c2, *args[1:]]
