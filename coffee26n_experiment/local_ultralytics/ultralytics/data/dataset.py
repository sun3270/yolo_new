...
class YOLODataset(BaseDataset):
    ...
    def build_transforms(self, hyp: dict | None = None) -> Compose:
        ...
        bgmix_p = float(getattr(hyp, "coffee_bgmix", 0.0)) if self.augment else 0.0
        bgmix_mode = str(getattr(hyp, "coffee_bgmix_mode", "sample_cpu"))
        if bgmix_mode not in {"sample_cpu", "batch_gpu"}:
            raise ValueError(f"coffee_bgmix_mode must be sample_cpu or batch_gpu, got {bgmix_mode!r}.")
        if bgmix_p and bgmix_mode == "sample_cpu":
            transforms.append(
                CoffeeBgMix(
                    p=bgmix_p,
                    box_expand=float(getattr(hyp, "coffee_bgmix_box_expand", 0.02)),
                    feather_px=int(getattr(hyp, "coffee_bgmix_feather_px", 16)),
                    blur_kernel=int(getattr(hyp, "coffee_bgmix_blur_kernel", 21)),
                    no_box=str(getattr(hyp, "coffee_bgmix_no_box", "skip")),
                )
            )
        transforms.append(
            Format(
