...
class CoffeeBgMix:
    """Training-only box-preserving background transform inserted before Format normalization."""
    ...
    def __init__(self, p: float, box_expand: float, feather_px: int, blur_kernel: int, no_box: str = "skip"):
        ...
        self.no_box = no_box
        # Shared counters remain visible from the main process when DataLoader workers execute this transform.
        self._stats = mp.Array("d", [0.0] * 6, lock=True)

    def snapshot_stats(self, reset: bool = False) -> dict[str, Any]:
        transforms = {0: None, 1: "blur", 2: "desaturate", 3: "contrast"}
        with self._stats.get_lock():
            values = list(self._stats)
            result = {
                "applied": int(values[0]),
                "skipped_no_boxes": int(values[1]),
                "skipped_probability": int(values[2]),
                "preserve_ratio": values[3] / values[4] if values[4] else 0.0,
                "last_transform": transforms.get(int(values[5])),
            }
            if reset:
                for index in range(len(self._stats)):
                    self._stats[index] = 0.0
        return result

    @property
    def stats(self) -> dict[str, Any]:
        return self.snapshot_stats(reset=False)

    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        from augmentations.bgmix import apply_bgmix

        image = np.ascontiguousarray(labels["img"])
        height, width = image.shape[:2]
        instances = deepcopy(labels["instances"])
        instances.convert_bbox(format="xyxy")
        instances.denormalize(width, height)
        boxes = torch.as_tensor(instances.bboxes, dtype=torch.float32)
        if boxes.numel() == 0 and self.no_box == "fail":
            raise ValueError("BgMix received an empty-label training image and no_box=fail.")
        tensor = torch.from_numpy(image).permute(2, 0, 1).to(torch.float32)
        output, info, _ = apply_bgmix(
            tensor,
            boxes,
            p=self.p,
        ...
        transform_codes = {None: 0, "blur": 1, "desaturate": 2, "contrast": 3}
        with self._stats.get_lock():
            if status == "applied":
                self._stats[0] += 1
            elif status == "skipped_no_boxes":
                self._stats[1] += 1
            elif status == "skipped_probability":
                self._stats[2] += 1
            self._stats[3] += float(info.get("preserve_ratio", 0.0))
            self._stats[4] += 1
            self._stats[5] = transform_codes.get(info.get("transform"), 0)
        if np.issubdtype(image.dtype, np.integer):
            limits = np.iinfo(image.dtype)
            output = output.round().clamp(limits.min, limits.max)
        labels["img"] = output.permute(1, 2, 0).cpu().numpy().astype(image.dtype, copy=False)
        labels["coffee_bgmix"] = info
        return labels
